    ...
    def __init__(self, 
        ...
        if Ra_stars is None:
            ...
            if reoptimize:
                ...
                for i in range(30):
                    for j in range(len(self.Ra_stars)):
                        distances=torch.norm(self.Ra_stars[j].reshape(1,-1,3)-self.Ra_stars[j].reshape(-1,1,3),dim=2)
                        if (torch.max(torch.min(distances+100*torch.eye(distances.shape[0]),dim=1)[0])>10):
                            Ras[j] = torch.rand(*Ras[j].shape, dtype=torch.float64) * 5
                            self.Ra_stars[j] = self.get_Ra_star(graph_builders[j], Ras[j], Zas[j], params_star)
            else:
    ...
    def optimize_Adam(self, epoch, params_0: Dict[str, FloatTensor]) -> FloatTensor:
        ...
        for step in tqdm(range(epoch)):
            ...
            scalar_loss, derivative_numpy = self.objective_function(
                params_numpy)
            if (np.isnan(derivative_numpy[0])):
